...
//! - `BackpressureController` — Monitors bus health and adjusts rate rate
...
impl TokenBatch {
    ...
    /// Serialize batch to bytes using rkyv (zero-copy on deserialization)
    ...
    /// Deserialize batch from rkyv bytes (zero-copy if possible)
    pub fn from_bytes_rkyv(bytes: &[u8]) -> Result<Self, BackpressureError> {
        unsafe {
            rkyv::from_bytes_unchecked::<TokenBatch, rkyv::rancor::Error>(bytes)
                .map_err(|e| BackpressureError::Deserialization(e.to_string()))
        }
    }

    /// Serialize batch to bytes (JSON - fallback compatibility)
    pub fn to_bytes(&self) -> Result<Vec<u8>, serde_json::Error> {
        serde_json::to_vec(self)
    }

    /// Deserialize batch from bytes (JSON - fallback compatibility)
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, BackpressureError> {
        serde_json::from_slice(bytes)
            .map_err(|e| BackpressureError::Deserialization(e.to_string()))
    }

    /// Get created_at timestamp (reconstructed from flush timing)
    pub fn created_at(&self) -> Instant {
        // For rkyv, we use approximation based on batch age
        Instant::now()
    }
}

    /// Serialize batch to bytes for transport (JSON)
    pub fn to_bytes(&self) -> Result<Vec<u8>, serde_json::Error> {
        serde_json::to_vec(self)
    }

    /// Deserialize batch from bytes (JSON)
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, BackpressureError> {
        let mut batch: TokenBatch = serde_json::from_slice(bytes)
            .map_err(|e| BackpressureError::Deserialization(e.to_string()))?;
        // Reset created_at since it was skipped during serialization
        batch.created_at = Instant::now();
        Ok(batch)
    }

    // Rkyv support requires Archive trait implementation (reserved for future)
    // pub fn to_bytes_rkyv(&self) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    //     unsafe {
    //         Ok(rkyv::to_bytes::<rkyv::rancor::Error>(self)?.into_vec())
    //     }
    // }
    //
    // pub fn from_bytes_rkyv(bytes: &[u8]) -> Result<Self, Box<dyn std::error::Error>> {
    //     unsafe {
    //         rkyv::from_bytes_unchecked::<TokenBatch, rkyv::rancor::Error>(bytes)
    //             .map_err(|e| e.into())
    //     }
