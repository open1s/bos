...
const { Bus, BusConfig } = require('../jsbos.js');
...
async function demoCallbackLoop() {
  ...
  // Note: Due to NAPI limitation, callback receives null as parameter.
  // Use workaround: check for null/undefined and handle accordingly.
  await sub.run((msg) => {
    // NAPI ThreadsafeFunction returns null when callback doesn't explicitly return
    // But the callback IS being called - we just receive null as the parameter
