 ...
 *
 * Usage: node performance_harness.js [--quick]
...
async function benchmarkStreaming(agent) {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        agent.stream('Say: ok', (err, token) => {
          ...
          if (token.type === 'Text') {
            if (firstToken) { ttfts.push(Number(now - startTime) / 1e6); firstToken = false }
            else if (lastTokenTime) { interTokenGaps.push(Number(now - lastTokenTime) / 1e6) }
            lastTokenTime = now
  ...
  if (ttfts.length > 0) {
    ...
    console.log(`\n  TTFT: Avg=${(ttfts.reduce((a, b) => a + b, 0) / ttfts.length).toFixed(1)}ms, P50=${sorted[Math.floor(sorted.length * 0.5)].toFixed(1)}ms`)
  }
  if (interTokenGaps.length > 0) {
    const sorted = [...interTokenGaps].sort((a, b) => a - b)
    console.log(`  Inter-Token Gap: Avg=${(interTokenGaps.reduce((a, b) => a + b, 0) / interTokenGaps.length).toFixed(2)}ms`)
  }
}
...
async function main() {
  ...
  console.log('  Summary')
  console.log('═'.repeat(60))
  if (results.e2e) console.log(`  E2E Avg Latency: ${results.e2e.avg.toFixed(0)}ms`)
  if (results.overhead) console.log(`  Engine vs Wall ratio: ${(results.overhead.totalEngineTimeUs / results.overhead.totalWallTimeUs * 100).toFixed(0)}%`)
  console.log('')
