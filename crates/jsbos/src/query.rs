...
pub struct Queryable {
    ...
    pub(crate) stream_handler: Arc<std::sync::Mutex<Option<Arc<ThreadsafeFunction<String, UnknownReturnValue>>>>>,
}
...
impl Queryable {
    ...
    #[napi]
    pub async fn run_stream(&self, handler: ThreadsafeFunction<String, UnknownReturnValue>) -> Result<()> {
        let mut guard = self.inner.lock().await;
